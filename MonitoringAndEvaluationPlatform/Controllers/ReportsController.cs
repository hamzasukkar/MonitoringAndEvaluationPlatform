        ...
        public async Task<IActionResult> Index()
        {
            ...
            viewModel.MinistryReports = ministries
                ...
                .Select(m => {
                    ...
                    return new CategoryReportItem
                    {
                        ...
                        AmountSpent = ministryProjects.Sum(p => p.Phases?.Where(pp => pp.ActionPlan != null).SelectMany(pp => pp.ActionPlan!.Plans).Sum(plan => plan.Realised) ?? 0),
                        IndicatorsPerformance = Math.Round(m.IndicatorsPerformance, 2),
                        DisbursementPerformance = Math.Round(m.DisbursementPerformance, 2)
                    };
                })
            ...
            viewModel.SectorReports = sectors
                ...
                .Select(s => {
                    ...
                    return new CategoryReportItem
                    {
                        ...
                        AmountSpent = sectorProjects.Sum(p => p.Phases?.Where(pp => pp.ActionPlan != null).SelectMany(pp => pp.ActionPlan!.Plans).Sum(plan => plan.Realised) ?? 0),
                        IndicatorsPerformance = Math.Round(s.IndicatorsPerformance, 2),
                        DisbursementPerformance = Math.Round(s.DisbursementPerformance, 2)
                    };
                })
            ...
            viewModel.DonorReports = donors
                ...
                .Select(d => {
                    ...
                    return new CategoryReportItem
                    {
                        ...
                        AmountSpent = donorProjects.Sum(p => p.Phases?.Where(pp => pp.ActionPlan != null).SelectMany(pp => pp.ActionPlan!.Plans).Sum(plan => plan.Realised) ?? 0),
            ...
            viewModel.SupervisorReports = supervisors
                .Select(s => {
                    ...
                    return new CategoryReportItem
                    {
                        ...
                        AmountSpent = supervisorProjects.Sum(p => p.Phases?.Where(pp => pp.ActionPlan != null).SelectMany(pp => pp.ActionPlan!.Plans).Sum(plan => plan.Realised) ?? 0),
            ...
            viewModel.ProjectManagerReports = projectManagers
                .Select(pm => {
                    ...
                    return new CategoryReportItem
                    {
                        ...
                        AmountSpent = pmProjects.Sum(p => p.Phases?.Where(pp => pp.ActionPlan != null).SelectMany(pp => pp.ActionPlan!.Plans).Sum(plan => plan.Realised) ?? 0),
            ...
            viewModel.GovernorateReports = governorates
                .Select(g => {
                    ...
                    return new CategoryReportItem
                    {
                        ...
                        AmountSpent = allGovProjects.Sum(p => p.Phases?.Where(pp => pp.ActionPlan != null).SelectMany(pp => pp.ActionPlan!.Plans).Sum(plan => plan.Realised) ?? 0),
            ...
            if (nationalProjects.Any())
            {
                viewModel.GovernorateReports.Insert(0, new CategoryReportItem
                {
                    ...
                    AmountSpent = nationalProjects.Sum(p => p.Phases?.Where(pp => pp.ActionPlan != null).SelectMany(pp => pp.ActionPlan!.Plans).Sum(plan => plan.Realised) ?? 0),
